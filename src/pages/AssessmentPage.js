...
const AssessmentSessionPage = ({ currentUserId, currentUser, onNavigate, onLogout }) => {
  ...
  const loadSessions = async () => {
    try {
      // Load sessions where current user is participant A or B
  ...
  const loadTeamMembers = async () => {
    try {
      ...
      // Filter to get team members (for now, show all users except self)
  ...
  const createSession = async () => {
    ...
    try {
      const newSession = {
        ...
        participantA: selectedMember, // Team member (ISF)
        participantB: currentUserId, // Leader (ISL)
        status: 'pending',
      ...
      setSelectedMember('');
      loadSessions();
      alert('Assessment session created! Share the link with your team member to begin.');
    } catch (error) {
  ...
  const startAssessment = (sessionId) => {
    // Navigate to assessment page with session ID
