...
// ✅ Updated with P2P publishing logic - Assessee publishes their own MSH
...
export default function ComparisonView360() {
  ...
  const [relationshipType, setRelationshipType] = useState(null); // 'peer' or 'manager'
  
  // Aligned scores (editable by publisher)
  ...
  const loadComparisonData = async () => {
    try {
      ...
      // Determine relationship type from first bilateral assessment
      const bilateralAssessment = assessments.find(a => 
        a.assessmentType !== 'self' && 
        (a.giver?.uid || a.assessorId) !== (a.receiver?.uid || a.subjectId)
      );
      
      const detectedRelationshipType = bilateralAssessment?.assessmentType === 'peer' ? 'peer' : 'manager';
      setRelationshipType(detectedRelationshipType);
      
      console.log('🔗 Relationship Type:', detectedRelationshipType);
      
      // Extract person IDs from pairingId format: 360-pair-cycle-YYYY-MM-userA-userB
      ...
      // Determine person A and person B IDs
      let personAId, personBId;
      
      if (detectedRelationshipType === 'peer') {
        // For P2P, use pairing ID structure
        personAId = userA;
        personBId = userB;
      } else {
        // For MR/DR, determine from assessment structure
        const sampleAssessment = assessments[0];
        personAId = sampleAssessment.assessmentType === 'manager-down' 
          ? sampleAssessment.assessorId 
          : sampleAssessment.subjectId;
        personBId = sampleAssessment.assessmentType === 'manager-down'
          ? sampleAssessment.subjectId
          : sampleAssessment.assessorId;
      }
      ...
      const detectedPairing = detect360Pairing(assessments, personAId, personBId, cycleMonth, cycleYear);
  ...
  const handlePublishMSH = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'mshScores'), {
        ...
        relationshipType: relationshipType // Track if this is peer or manager review
      });
  ...
  // Determine who can edit/publish based on relationship type
  const canEdit = relationshipType === 'peer'
    ? user?.uid === currentPair.bilateralAssessment?.subjectId  // P2P: Assessee publishes their own
    : user?.uid === pairing.mrId;                               // MR/DR: MR publishes both
  
  const isPublished = currentPair.bilateralAssessment?.mshPublished;
  
  // Determine publisher name for display
  const publisherName = relationshipType === 'peer' 
    ? subjectName + ' (Assessee publishes own MSH)'
    : names.mrName + ' (Manager publishes both MSHs)';
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <p className="text-blue-100">
            ...
            {relationshipType === 'peer' && ' • Peer-to-Peer'}
          </p>
          ...
          {!isPublished && !canEdit && (
            <div className="mt-3 bg-yellow-500 text-white px-4 py-2 rounded-lg inline-flex items-center gap-2">
              <span className="font-semibold">View Only - {publisherName}</span>
            </div>
          )}
        </div>
      </div>
        ...
        {canEdit && !isPublished && (
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="mt-4 text-center text-sm text-gray-600">
              ...
              {relationshipType === 'peer' && ' • As the assessee, you publish your own MSH'}
            </div>
          </div>
        )}
