...
// ✅ RENAMED FROM: UnifiedAssessmentGrid.js
// The Universal Assessment Orchestrator - One Grid for All Hubs
// ✅ FIXED: pairComplete now checks for both 'completed' AND 'not-aligned' status
// ✅ FIXED: MR/DR Manager publishes BOTH Pair A and Pair B MSHs
...
function AssessmentOrchestrator({ 
  ...
  const isAssessmentComplete = (status) => {
    return status === 'completed' || status === 'not-aligned' || status === 'calibrated';
  };

  // ✅ FIXED: Render a single 360° Pair Card with correct pairComplete logic
  const renderPairCard = (pairing, pairType, subjectPerson, assessorPerson, selfStatus, assessmentStatus, assessmentId, publisherUid, publisher) => {
    // ✅ CRITICAL FIX: Accept BOTH 'completed' AND 'not-aligned' as complete statuses
    ...
    console.log(`🎯 renderPairCard - ${pairLabel}:`, {
      subjectPerson: subjectPerson?.displayName,
      selfStatus,
      assessmentStatus,
      pairComplete,
    ...
    // ✅ FIXED: Create navigation object with pairId and selectedPair
    const handleNavigation = () => {
      console.log('🎯 Orchestrator - Button clicked!');
      console.log('  pairId:', pairing.pairId);
      console.log('  selectedPair (pairType):', pairType);
      console.log('  subjectPerson:', subjectPerson?.displayName);
      console.log('  assessorPerson:', assessorPerson?.displayName);
      
      if (!onView360Pair) {
        console.error('❌ onView360Pair is not defined!');
        return;
      }
      ...
      console.log('  Calling onView360Pair with:', navObject);
      onView360Pair(navObject);
      console.log('  ✅ onView360Pair called');
    };
    ...
    return (
      <div className="space-y-6">
        {pairings.map((pairing) => {
          ...
          // ✅ CRITICAL FIX: Determine publisher ONCE for the entire pairing
          // P2P: Subject publishes their own MSH (determined per card)
          // MR/DR: Manager publishes BOTH MSHs (same for both cards)
          ...
          if (isPeerPairing) {
            ...
            // MR/DR: Find the manager - they publish BOTH Pair A and Pair B
            publisherUid = pairing.managerId;
            
            // Find which person is the manager
            if (personA?.uid === pairing.managerId) {
              publisher = personA;
            } else if (personB?.uid === pairing.managerId) {
              publisher = personB;
            } else {
              // Fallback: Check by layer (ISE or ISL is manager)
              if (personA?.layer === 'ISE' || personA?.layer === 'ISL') {
                publisher = personA;
            ...
            console.log('🎯 MR/DR Pairing Publisher:', {
              pairId: pairing.pairId,
              managerId: pairing.managerId,
              publisher: publisher?.displayName,
              publisherUid,
              personA: personA?.displayName,
              personB: personB?.displayName
            });
          }
          ...
          // For P2P, determine publisher per card
          const pairA_publisher = isPeerPairing ? personB : publisher;
          const pairA_publisherUid = isPeerPairing ? personB?.uid : publisherUid;
          
          const pairB_publisher = isPeerPairing ? personA : publisher;
          const pairB_publisherUid = isPeerPairing ? personA?.uid : publisherUid;
            ...
            <div key={pairing.pairId} className="bg-white rounded-lg shadow-sm border-2 border-gray-200 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {renderPairCard(
                  ...
                  pairA_subjectSelf,
                  pairA_assessment,
                  pairing.assessmentIds?.personA_to_B,
                  pairA_publisherUid,
                  pairA_publisher
                )}
                ...
                {renderPairCard(
                  ...
                  pairB_subjectSelf,
                  pairB_assessment,
                  pairing.assessmentIds?.personB_to_A,
                  pairB_publisherUid,
                  pairB_publisher
                )}
              </div>
